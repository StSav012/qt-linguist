...
def find_translation_calls(
    ...
    def message_from_translate_call(
        line: int,
        context: str,
        sourceText: str,
        ...
        def relative_path(_path: Path, _base_path: Path) -> Path:
            ...
            while (
                _base_path_parts
                and _path_parts
                and _path_parts[0] == _base_path_parts[0]
            ):
                del _path_parts[0]
    ...
    def check_expression(
        operator: ast.expr, class_name: str = ""
    ) -> Iterator[TranslatorMessage]:
        field: str
        ...
        if isinstance(operator, ast.Call):
            if (
                ...
                and operator.func.id in import_translate_as
            ):
                yield message_from_translate_call(
                    ...
                    disambiguation=(
                        ...
                        and isinstance(operator.args[2], ast.Constant)
                        else (
                    ...
                    n=(
                        cast(ast.Constant, operator.args[3]).value
                        if len(operator.args) > 3
                        and isinstance(operator.args[3], ast.Constant)
                        else -1
                    ),
                )
            elif (
                class_name
                and isinstance(operator.func, ast.Attribute)
                ...
                yield message_from_translate_call(
                    ...
                    disambiguation=(
                        ...
                        and isinstance(operator.args[1], ast.Constant)
                        else (
    ...
    def walk_body(
        body: list[ast.stmt] | list[ast.excepthandler], class_name: str = ""
    ) -> Iterator[TranslatorMessage]:
        ...
        for item in body:
            ...
            if isinstance(item, ast.ImportFrom):
                ...
                if item.module is not None and item.module.endswith(".QtCore"):
                    for if_name in item.names:
                        if (if_name.asname or if_name.name) in (
                            "QCoreApplication",
                            "QGuiApplication",
                            "QApplication",
                        ):
                            import_qt_core_application_as.add(
                                if_name.asname or if_name.name
                            )
            if isinstance(item, (ast.AnnAssign, ast.Assign)):
                ...
                if isinstance(item.value, ast.Attribute) and isinstance(
                    target, ast.Name
                ):
                    ...
                    if r_value.attr == "translate":
                        ...
                        if isinstance(r_name, ast.Attribute):
                            if (
                                r_name.attr == "QCoreApplication"
                                and r_name.value.id in import_qt_core_as
                            ):
                                import_translate_as.add(target.id)
                            if (
                                r_name.attr == "QGuiApplication"
                                and r_name.value.id in import_qt_gui_as
                            ):
                                import_translate_as.add(target.id)
                            if (
                                r_name.attr == "QApplication"
                                and r_name.value.id in import_qt_widgets_as
                            ):
                                import_translate_as.add(target.id)
                if isinstance(item.value, ast.Tuple) and isinstance(target, ast.Tuple):
                    for l_el, r_el in zip(target.elts, item.value.elts):
                        if isinstance(r_el, ast.Attribute) and isinstance(
                            l_el, ast.Name
                        ):
                            ...
                            if r_el.attr == "translate":
                                ...
                                if isinstance(r_el_name, ast.Attribute):
                                    if (
                                        r_el_name.attr == "QCoreApplication"
                                        and r_el_name.value.id in import_qt_core_as
                                    ):
                                        import_translate_as.add(l_el.id)
                                    if (
                                        r_el_name.attr == "QGuiApplication"
                                        and r_el_name.value.id in import_qt_gui_as
                                    ):
                                        import_translate_as.add(l_el.id)
                                    if (
                                        r_el_name.attr == "QApplication"
                                        and r_el_name.value.id in import_qt_widgets_as
                                    ):
                                        import_translate_as.add(l_el.id)
...
Qt TS XML). New and modified messages can be merged into existing TS files.""",
    ...
    ap.add_argument(
        "-version",
        action="version",
        version=f"lupdate version {qVersion()}",
        help="Display the version of lupdate and exit.",
    )
    ap.add_argument(
        "-no-obsolete",
        action="store_true",
        help="Drop all obsolete and vanished strings.",
    )
    ap.add_argument(
        "-recursive",
        action=argparse.BooleanOptionalAction,
        help="Recursively scan directories (default).",
    )
    ap.add_argument(
        "-ext",
        nargs="+",
...
Default is absolute for new files.""",
    ...
    for fn in sources:
        current_messages.extend(
            find_translation_calls(fn.absolute(), base_path=ts_path)
        )
    ...
    for cur_msg in current_messages:
        if (
            ...
            cur_msg.sourceText(),
            cur_msg.comment(),
        ) not in new_references:
            new_references[
                (cur_msg.context(), cur_msg.sourceText(), cur_msg.comment())
            ] = []
        if (filename := cur_msg.fileName()) is not None:
            new_references[
                (cur_msg.context(), cur_msg.sourceText(), cur_msg.comment())
            ].append(TranslatorMessage.Reference(filename, cur_msg.lineNumber()))
    for cur_msg in current_messages:
        if (
            cur_msg.context(),
            cur_msg.sourceText(),
    ...
    printOut(
        QString(
            QCoreApplication.translate(
                "LUpdate",
                "    Generated %n translation(s) (%1 obsolete and %2 actual)",
                None,
                tor.messageCount(),
            )
        ).arg(len(init_messages), len(new_messages))
    )

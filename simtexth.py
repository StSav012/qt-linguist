...
class Candidate:
    ...
    def __eq__(self, other: 'Candidate') -> bool:
        return (self.translation == other.translation and self.source == other.source and self.context == other.context
...
class CoMatrix:
    ...
    def __init__(self, string: str = '') -> None:
        ...
        for i, d in enumerate(ba):
            self.setCoOccurence(c, d)
            if i < len(ba) - 1:
                c = ba[i + 1]
                self.setCoOccurence(d, c)

    def setCoOccurence(self, c: int, d: int) -> None:
...
def similarTextHeuristicCandidates(tor: Translator, text: str, maxCandidates: int) -> CandidateList:
    scores: list[int] = []
    ...
    for mtm in tor.messages():
        if (mtm.type() == TranslatorMessage.Type.Unfinished
                or not mtm.translation()):
            continue
        ...
        if len(candidates) < maxCandidates and score >= textSimilarityThreshold:
            cand: Candidate = Candidate(mtm.context(), s, mtm.comment(), mtm.translation())

...
def releaseTranslator(
    ...
    if remove_identical:
        if cd.isVerbose():
            printOut(
                f"Removing translations equal to source text in '{qm_filename}'...\n"
            )
...
format into the 'compiled' QM format used by QTranslator objects.""",
    ...
    ap.add_argument(
        "-compress", action=argparse.BooleanOptionalAction, help="Compress the QM files"
    )
    ap.add_argument(
        "".join(("-no", "unfinished")),
        action="store_true",
    ...
    ap.add_argument(
        ...
        help=" ".join(
            (
                "If a message has no real translation,",
                "use the source text prefixed with the given string instead",
            )
        ),
    )
    ap.add_argument(
    ...
    ap.add_argument(
        "-version",
        action="version",
        version=f"lrelease version {qVersion()}",
